...
R"rawliteral(";
  ...
  function renderTelemetry(data) {
    ...
    card.innerHTML = '<b>' + names[id] + '</b> | Bat: <span style="color:' + batColor + '">' + data.bat + '%</span><br>' +
      ...
      'Fix: ' + (!data.fix ? '<span style="color:#f55">NO FIX</span>' : data.fresh ? '<span style="color:#0f9">FRESH</span>' : '<span style="color:#fa0">STALE</span>') + '<br>' +
      'Last: <span id="seen-' + id + '">just now</span>';
    card.style.outline = followingId == id ? '1px solid #0af' : 'none';

    if (data.fix && data.fresh) {
  ...
  function startRun() {
    ...
    Object.keys(liveTrackers).forEach(function(id) { liveTrackers[id].points = []; });
    var runId = makeRunId();
  ...
  setInterval(function() {
    Object.keys(lastSeen).forEach(function(id) {
      ...
      var tr = trackerRenders[id];
      if (tr && tr.marker) tr.marker.getElement().style.opacity = stale ? '0.4' : '1';
    });
  }, 1000);
